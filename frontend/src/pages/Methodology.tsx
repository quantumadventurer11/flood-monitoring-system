      ...
      <div className="space-y-5">
        ...
        <Section id="validation" title="Independent Validation Source">
          <div className="grid gap-4 md:grid-cols-[1fr_220px]">
            <div className="space-y-2 text-sm text-slate-700">
              <p className="font-semibold text-slate-900">{data.independent_validation.source.name}: {data.independent_validation.source.title}</p>
              ...
              <p className="text-xs text-slate-500">{data.independent_validation.source.caveat}</p>
              <div className="flex flex-wrap gap-2 pt-1">
                ...
                <a className="rounded border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700" href={data.independent_validation.source.shapefile_url} target="_blank" rel="noreferrer">Shapefile</a>
              </div>
            </div>
            <dl className="rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm">
              <dt className="text-slate-500">Patch labels</dt>
              <dd className="text-2xl font-bold text-slate-900">{data.independent_validation.ground_truth.patches}</dd>
              <dt className="mt-3 text-slate-500">Flooded patches</dt>
              <dd className="text-2xl font-bold text-blue-700">{data.independent_validation.ground_truth.flooded_patches}</dd>
              <dt className="mt-3 text-slate-500">Flooded share</dt>
              <dd className="text-2xl font-bold text-blue-700">{data.independent_validation.ground_truth.flooded_percent}%</dd>
            </dl>
          </div>
        </Section>
        <Section id="references" title="Methodology References" caption="External papers used to frame forecasting, uncertainty, and validation language.">
          <div className="space-y-3">
            {data.methodology_references.map((reference) => (
              <article key={reference.key} className="rounded-lg border border-slate-200 bg-white p-4 text-sm">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <h3 className="font-semibold text-slate-900">{reference.title}</h3>
                    <p className="mt-1 text-slate-600">{reference.authors} ({reference.year}). {reference.venue}.</p>
                  </div>
                  <a className="rounded border border-slate-300 px-3 py-1.5 text-xs font-semibold text-slate-700" href={`https://doi.org/${reference.doi}`} target="_blank" rel="noreferrer">DOI</a>
                </div>
                <p className="mt-3 text-slate-700">{reference.relevance}</p>
                <div className="mt-3 rounded border border-slate-200 bg-slate-50 p-3">
                  <p className="text-xs font-semibold uppercase text-slate-500">Copy-ready APA citation</p>
                  <p className="mt-1 text-slate-800">{reference.citation}</p>
                </div>
              </article>
            ))}
          </div>
        ...
        <Section id="modularity" title="Modularity Evidence">
          <ul className="grid gap-3 text-sm text-slate-700 md:grid-cols-3">
            {data.modularity_evidence.map((item) => <li key={item} className="rounded-lg border border-slate-200 bg-white p-3">{item}</li>)}
          </ul>
        </Section>
